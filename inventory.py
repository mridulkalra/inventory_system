...
def Database():
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS `product` (product_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, product_name TEXT, product_qty INTEGER, product_price INTEGER)")
...
def LoginForm():
    ...
    username = Entry(MidLoginForm, font=('arial', 25), width=15)

    username.grid(row=0, column=1)
    password = Entry(MidLoginForm, font=('arial', 25), width=15, show="*")
...
def Home():
    ...
    filemenu2.add_command(label="Delete Items", command=delete_items)
...
def warning():
    ...
    a = 5
    str="the "
    cursor.execute("select product_name,product_qty from product where product_qty<=%d" % (a))
    data = cursor.fetchall()
    list1=list()
    for i in data:
        list1.append(i[0])
    list2=[" is going out of stock"]
    for i in list1:
        str=str+" "+i

    tkMessageBox.showwarning("Warning",str+" "+list2[0])
    conn.commit()
...
def delete_items():
    ...
    global productname1
    global productqty1
    deleteitems = Toplevel()
    deleteitems.title("Simple Inventory System/Add new")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
    x = (screen_width / 2) - (width / 2)
    y = (screen_height / 2) - (height / 2)
    deleteitems.geometry("%dx%d+%d+%d" % (width, height, x, y))
    deleteitems.resizable(0, 0)
    TopAddNew = Frame(deleteitems, width=600, height=100, bd=1, relief=SOLID)
    TopAddNew.pack(side=TOP, pady=20)
    lbl_text = Label(TopAddNew, text="Delete Items", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
    MidAddNew = Frame(deleteitems, width=600)
    MidAddNew.pack(side=TOP, pady=50)
    lbl_productname = Label(MidAddNew, text="Product Id:", font=('arial', 25), bd=10)
    lbl_productname.grid(row=0, sticky=W)
    lbl_qty = Label(MidAddNew, text="No. of items:", font=('arial', 25), bd=10)
    lbl_qty.grid(row=1, sticky=W)
    productname1 = Entry(MidAddNew, font=('arial', 25), width=15)
    productname1.grid(row=0, column=1)
    productqty1 = Entry(MidAddNew, font=('arial', 25), width=15)
    productqty1.grid(row=1, column=1)
    btn_delete = Button(MidAddNew, text="Save", font=('arial', 18), width=30, bg="#009ACD",command=del_items)
    btn_delete.grid(row=3, columnspan=2, pady=20)


def del_items():
    Database()
    cursor.execute("select product_qty from product where product_id=%d" %(int(productname1.get())))
    qty=cursor.fetchall()
    print(qty)
    cursor.execute('update product set product_qty=%d where product_id=%d' % (qty[0][0]-int(productqty1.get()), int(productname1.get())))
    conn.commit()
    cursor.close()
    conn.close()
    deleteitems.destroy()
...
def AddNewForm():
    def AddNew():
        ...
        cursor.execute("INSERT INTO `product` (product_name, product_qty, product_price) VALUES(?, ?, ?)",
                       (str(productname.get()), int(productqty.get()), int(productprice.get())))
        conn.commit()
        cursor.close()
        conn.close()
        addnewform.destroy()
    ...
    productname = Entry(MidAddNew, font=('arial', 25), width=15)
    productname.grid(row=0, column=1)
    productqty = Entry(MidAddNew, font=('arial', 25), width=15)
    productqty.grid(row=1, column=1)
    productprice = Entry(MidAddNew, font=('arial', 25), width=15)
...
def ViewForm():
    ...
    SEARCH = Entry(LeftViewForm, font=('arial', 15), width=10)
